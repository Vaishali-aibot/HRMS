...
import {
  ...
  type ResignationActionState,
} from "@/lib/actions/resignation";
...
export function ResignationRequestRow({
  request,
    ...
    <li className="rounded-md border border-black/10 p-3 text-sm dark:border-white/15">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          Resigning {request.resignationDate} · {request.noticePeriodDays}-day notice
        </div>
      ...
      {request.reason && (
        <p className="mt-1 text-black/60 dark:text-white/60">{request.reason}</p>
      )}
